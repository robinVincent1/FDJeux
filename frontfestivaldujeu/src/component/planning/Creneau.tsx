...
const Creneau: React.FC<CreneauProps> = ({
  ...
  const [list_benevole, setListBenevole] = React.useState<Users[]>([]);
  const [idJour, setIdJour] = React.useState<number>(JourId);
  const [currentIdCreneau, setCurrentIdCreneau] =
    React.useState<number>(idCreneau);
  ...
  const [referentlist, setreferentlist] = React.useState<Users[]>([]);
  const [thereferentid, setthereferentid] = React.useState<number>(ReferentId);
  const [thereferent, setthereferent] = React.useState<Users>();
  const [list_flexible, setlistflexible] = React.useState<Users[]>([]);
  ...
  useEffect(() => {
    ...
    getreferentlist().then((result) => setreferentlist(result as Users[]));
    getthereferent().then((result) => setthereferent(result as Users));
    getflexiblebenevole().then((result) => setlistflexible(result as Users[]));
  ...
  async function getflexiblebenevole() {
    try {
      const response = await fetch(
        `http://localhost:8080/user/flexible/${idFestival}`,
        {
          method: "GET",
          headers: {
      ...
      const flexiblebenevole: Users[] = await response.json();
  ...
  async function getreferentlist() {
    try {
      const responseReferent = await fetch(
        `http://localhost:8080/user/referent/${idFestival}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      const referentList: Users[] = await responseReferent.json();
      return referentList;
    } catch (error) {
      console.error(error);
    }
  ...
  async function getthereferent() {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/user/${thereferentid}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      const referent: Users = await response.json();
      return referent;
    } catch (error) {}
  }
  ...
  async function changereferent() {
    try {
      const reponse = await fetch(
        `http://localhost:8080/creneau/modifyreferent/${currentIdCreneau}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  async function getNomJour(id: number) {
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          "Erreur lors de la récupération du jour. Statut:",
          response.status
        );
      }
  ...
  async function addnbinscrit(nb_inscrit: number) {
    try {
      const reponse = await fetch(
        `http://localhost:8080/creneau/addnbinscrit/${currentIdCreneau}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  async function getbenevole(idCreneau: number) {
    try {
      ...
      if (!response.ok) {
        // Si la réponse n'est pas OK, renvoyer une valeur appropriée
        console.error(
          "Erreur lors de la récupération des id des users. Statut:",
          response.status
        );
      }
  ...
  async function fillListBenevole() {
    const benevoleList:
      ...
      | undefined = await getbenevole(currentIdCreneau);
    ...
    if (benevoleList) {
      for (let i = 0; i < benevoleList.length; i++) {
        ...
        if (!response.ok) {
          console.error(
            "Erreur lors de la récupération de l user. Statut:",
            response.status
          );
        }
  ...
  async function subnbinscrit(nb_inscrit: number) {
    try {
      const reponse = await fetch(
        `http://localhost:8080/creneau/subnbinscrit/${currentIdCreneau}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  async function changeOuvert() {
    try {
      const reponse = await fetch(
        `http://localhost:8080/creneau/modifyouvert/${currentIdCreneau}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  async function changeNbMax() {
    try {
      const reponse = await fetch(
        `http://localhost:8080/creneau/modifynbmax/${currentIdCreneau}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
        ...
        >
          <ModalDialog color="neutral" variant="plain">
            ...
            <Typography id="modal-modal-title" variant="h6" component="h2">
              {ouvert ? "Ouvert" : "Fermé"} / {titre} / {nomjour} {heure_debut}
              H-{heure_fin}H ({nb_inscrit}/{nb_max})
            </Typography>
            <Typography id="modal-modal-description" sx={{ mt: 2 }}>
              <Typography>
                <IconButton aria-label="delete" disabled color="primary">
                  {list_benevole.map((benevole) => (
                    <Typography
                      style={{ color: benevole.isPresent ? "green" : "red" }}
                    >
                      {benevole.pseudo}
                    </Typography>
                  ))}
              ...
              <Typography>
                Referent : {thereferent?.pseudo} {thereferent?.telephone}
              </Typography>
            </Typography>
              ...
              <ModalDialog color="neutral" variant="plain">
                ...
                {list_flexible.map((benevole) => (
                  <div>
                    ...
                    <Button
                      onClick={() => {
                        inscription(benevole.idUser, idCreneau, nb_inscrit);
                        handleCloseFlexible();
                      }}
                    >
                      +
            ...
            {ouvert ? (
              isInscrit ? (
                <Button
                  onClick={() =>
                    desinscription(userId(), idCreneau, nb_inscrit)
                  }
                >
                  Se désinscrire
                </Button>
                ...
                <Button
                  onClick={() => inscription(userId(), idCreneau, nb_inscrit)}
                >
                  S'inscrire
                </Button>
              )
            ...
            >
              <ModalDialog color="neutral" variant="plain">
                ...
                <FormControl fullWidth>
                  ...
                  >
                    <MenuItem key={1} value={"Ouvert"}>
                      Ouvert
                    </MenuItem>
                    <MenuItem key={0} value={"Fermé"}>
                      Fermé
                    </MenuItem>
                  </Select>
                  <TextField
                  ...
                  >
                    {referentlist.map((referent) => (
                      <MenuItem key={referent.idUser} value={referent.idUser}>
                        {referent.pseudo}
                      </MenuItem>
                    ))}
                  </Select>
