...
const PlanningGeneral: React.FC<PlanningGeneralProps> = ({ PlanningId }) => {
  ...
  const [listUserPresent, setListUserPresent] = useState<Users[]>([]);
  ...
  async function ChangePresence(jour: Jour, user: Users, isPresent: number) {
  ...
  async function getUserIdByCreneauId(idCreneau: number) {
    try {
      ...
      const ListUserId: Users[] = await response.json();
      return ListUserId;
    } catch (error) {
      console.error("Erreur lors de la récupération des creneaux", error);
      // Gérer l'erreur ici, si nécessaire
  ...
  async function getcreneaubyJourId(jourId: number) {
    try {
      const response = await fetch(
        `http://localhost:8080/creneau/${jourId}/${PlanningId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      for (let j = 0; j < list_jours.length; j++) {
        for (let k = 0; k < list_jours[j].list_horaire.length; k++) {
          const creneauData = await getcreneaubyId(
            list_jours[j].id,
            list_jours[j].list_horaire[k].id,
          ...
          if (creneauData == undefined) {
            await createcreneau(
              ...
              list_ligne[i].idPlanningGeneralLigne,
              list_ligne[i].titre,
              list_jours[j].list_horaire[k].heure_debut,
              list_jours[j].list_horaire[k].heure_fin
            );
          }
          if (creneauData) {
        ...
        <table className="w-full text-sm text-left rtl:text-right text-blue-100 dark:text-blue-100">
          ...
          <thead className="text-xs text-white uppercase bg-blue-600 border-b border-blue-400 dark:text-white">
            <tr>
              ...
              {list_jours.map((jour) => (
                ...
                >
                  ...
                  <div>
                    <Button
                      onClick={() =>
                        userConnected.role == "accueil bénévole" ||
                        (userConnected.role == "admin" &&
                          handleOpenModal_Presence(jour.id))
                      }
                      color="danger"
                    >
                      Présence
                    ...
                    >
                      <ModalDialog color="neutral" variant="plain">
                            ...
                            <FormGroup>
                              <FormControlLabel
                                control={
                                  <Switch
                                    color="success"
                                    checked={presentornot(user.isPresent)}
                                    onChange={() => {
                                      ChangePresence(
                                        jour,
                                        user,
                                      ...
                                      setListUserPresent([...listUserPresent]);
                                    }}
                                  />
                                }
                                label="Présent"
                        ...
                        <Button
                          color="danger"
                          onClick={() => handleCloseModal_Presence(jour.id)}
                        >
                          Fermé
                        </Button>
                      </ModalDialog>
                  ...
                  <div>
                    ...
                    >
                      <ModalDialog color="neutral" variant="plain">
                        ...
                        <Button
                          color="danger"
                          onClick={() => handleCloseModal_Horaire(jour.id)}
                        >
                          Fermé
                        </Button>
                        <Button
              ...
              {userConnected.role == "admin" && (
                <th>
                  <Button onClick={handleOpenModal_Jour} color="danger">
                    Ajouter un jour
                  </Button>
                  <Modal
                    open={openModal_Jour}
          ...
          <tbody>
            ...
            {userConnected.role == "admin" && (
              <Button onClick={handleOpenModal_Ligne} color="danger">
                Ajouter une ligne
              </Button>
            )}
            <Modal
